...
This file provides the classes for definition and sicretization of nonconvex domains.
...
class PolygonDomain2D(Domain):
    ...
    def boundaryLims(self, vertices):
        ...
        Output [bIndNum x 2 x 2]: array containing the boundary limits for each
            vertex in one row
        """
    ...
    def getMesh(self, discNum=100, bDiscNum=50, rfrac=0, sortflg=True):
        """
        ...
        he, coordinates = self.innerDisc(discNum, rfrac, sortflg)
    ...
    def innerDisc(self, discNum, rfrac=0., sortflg=True):
        """
        ...
        for d in range(2):
            ...
            coord1 = np.random.uniform(lim[0,d]+h, lim[1,d]-h, dof1)    # random test function locations
            coord2 = np.linspace(lim[0,d]+h, lim[1,d]-h, dof2)          # uniform test function locations
...
#        isInDom = self.isInside(coord, tol=min(he))                     # check that the points are in domain
    ...
    def boundaryDisc(self, vertices, bDiscNum, rfrac=0, sortflg=True):
        ...
        for i in range(sideNum):                            # loop over sides of the polygon
            discNum = np.ceil(bDiscNum*sideLen[i])          # number of discretization points
            bdof.append(discNum)
    ...
    def meshTot(self, discNum = 100, bDiscNum = 50):
        """
        This function creates a structured mesh of the polygon domain and discards
        points that fall within obstacles or outside the domain boundaries.
        The difference with the getMesh() file is that this function generates 
        ...
        class for details.
        """
        raise ValueError('This function needs to be defined before use!')
...
class Domain1D(Domain):
    ...
    def getMesh(self, discNum=100, bDiscNum=None, rfrac=0, sortflg=True):
        """
        This function creates a structured mesh of the polygon domain and discards
        points that fall within obstacles or outside the domain boundaries.
        ...
        dof1 = math.floor(dof*rfrac)                                    # random grid
        dof2 = dof - dof1                                               # uniform grid
        coord1 = np.random.uniform(lim[0]+he, lim[1]-he, dof1)          # random test function locations
        coord2 = np.linspace(lim[0]+he, lim[1]-he, dof2)                # uniform test function locations
        coordinates = np.hstack([coord1, coord2])                       # stack together the random and unform discretizations
        if rfrac>0 and sortflg:
